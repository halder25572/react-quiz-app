                ...
                <div className="text-blue-600 mb-5 text-base sm:text-lg dark:text-gray-400 md:grid md:grid-rows-2 md:grid-flow-col md:gap-2">
                    {
                        ...
                        options.map(list => <li list={list} onClick={() => quizCorrect(list)} key={list.id} className='list-none border py-8 m-4 rounded' ><input className = "mr-8 cursor-pointer" type="radio" name='radio'/>{list}</li>)
                    }
                </div>
        </div>
    </div>
